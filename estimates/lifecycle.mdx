...
| Sold | any other status | Requires management-level permissions: Sales Admin, Ops Manager, Admin, or Super Admin. |
...
- **Sales Admin, Admin, Super Admin** — full access to all statuses, with no per-status restriction.

Attempting to set a status the role is not allowed to set is rejected.

### Leaving Sold requires management-level permissions

Once an estimate is Sold, moving it to any other status requires management-level permissions: Sales Admin, Ops Manager, Admin, or Super Admin. A Sales Member can mark an estimate Sold but cannot later move it out of Sold.
...
### The effective sold date defaults to the first sold date and can be adjusted by Branch Management

The effective sold date is the date used for sales reporting. It starts out equal to the first sold date. It can later be adjusted, but only by Branch Management roles (Sales Admin and above), and only while the estimate is Sold. Adjusting it on an estimate that is not Sold is rejected.
...
Crew notes can be edited by Sales Member, Crew Leader, Ops Manager, Sales Admin, Admin, and Super Admin. Note that Crew Leader can edit crew notes even though they cannot otherwise edit estimates.
...
- **Effective sold date permissions.** The effective sold date is governed both at the field level (Branch Management) and at the action level (management-level permissions). These role sets are intended to line up (Sales Admin and above); keep them aligned if roles are ever added.
