...
- **How it works:** Tax is calculated only when the property address is in Washington state. When it applies, the tax service computes tax on the **final price** (the discounted price), using the property address. The total tax amount is the **sum of the state and local components** — state plus city, county, and special-district tax — computed for that address.
...
- **How it works:**
  ...
  - **Total tax** = the state tax amount + the local tax amount — it always equals the sum of the breakdown lines shown on the estimate.
