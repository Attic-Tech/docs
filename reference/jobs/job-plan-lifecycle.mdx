...
There is no separate "approver" role or approve permission. Approving is simply moving a submitted plan to **Approved**, and anyone who can edit the plan can do it. The person who approves is recorded automatically from who is signed in — it is not chosen from a list.
...
The plan is built on the job's plan page. The job must already exist and have a work order.
...
Each day needs at least one employee, every employee with valid hours greater than zero, and at least one work item with a quantity greater than zero. The matching messages are:
...
## The plan card on the job page

The job's plan card shows status-specific copy and a call to action:
...
- **Approved** — "Plan Approved" / **View Plan** (shows approver and approved date)
...
- **Approving** records the approver automatically and needs no special permission.
