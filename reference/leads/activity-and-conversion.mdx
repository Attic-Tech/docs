...
Because a first-time conversion creates a project, publishing an activity for a lead that has **not yet been converted** additionally requires a role that can create projects: **Admin**, **Sales Admin**, or **Sales Member**. Sales Members can publish their own assigned activities. **Ops Manager** and **Client Coordinator** can create and edit activities, but they can only publish for leads that are **already converted** — publishing an unconverted lead fails for them because it would need to create a project.
