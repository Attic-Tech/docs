...
- An address
- A project name
- The "client confirmed" indicator
...
Any role that can create or edit an activity and set it to Published can trigger conversion. Sales Members can publish their own assigned activities.

Important: if conversion fails for any reason, the failure is logged but the **activity publish still succeeds** from your point of view. In that case the client/project may not have been created (see Return behavior).
...
Because conversion errors are swallowed (see Publishing converts the lead), a publish can report no created project even though the activity itself was published. The app handles this case gracefully.
