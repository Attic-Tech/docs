...
- **Who can (by role):** **Admin, Ops Manager, Sales Admin, Client Coordinator.** **Sales Member and Crew Leader** have no lead permissions and see no button.
...
- **Cause:** Common reasons, in order:
  ...
  2. **Your session has no current branch** → "Branch not found." Creating a lead requires you to have a current branch.
...
- **Cause:** The selected user must exist ("The selected client coordinator does not exist."), belong to the organization ("…does not belong to this organization."), and **hold the Client Coordinator role** ("The selected user does not have the required role to be a client coordinator."). Also, the **lead must already have a branch**, or you'll see: *"A branch must be assigned to the lead before setting a client coordinator."*
...
- **Symptom:** Save fails: "The selected user does not have the required role to be a sales person."
...
- **Who can (by role):** Reliably **Admin** and **Ops Manager** (Branch Management). Sales Admin and Client Coordinator can manage leads in general, but lead deletion is restricted to management roles, so use Admin or Ops Manager to be certain.
- **Side-effect:** The lead is soft-deleted (it stops appearing in lists but isn't permanently erased).
- **Resolution:** Use a management-level role (Admin or Ops Manager), and be a member of the lead's branch.
...
| Open Clients page | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| Create / edit lead | ✓ | ✓ | ✓ | ✗ | ✗ | ✓ |
| Delete lead | ✓ | ✓ | restricted | ✗ | ✗ | restricted |
| Edit client | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| Delete client | ✓ | ✓ | ✓ | ✗ | ✗ | ✗ |
...
Note: lead and client **deletion** is reliably available to management roles (Admin, Ops Manager). For Sales Admin and Client Coordinator, deletion may be more restricted than other actions, so use a management role when you need to delete with certainty.
