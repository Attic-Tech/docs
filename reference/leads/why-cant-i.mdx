...
- **Cause:** Creating a project requires a sales role, and the client must exist in your organization. When you create a project from the client page, you set its primary contact in the create form — you can add a new one there — so a missing contact won't block you. When a project is created automatically (for example by converting a lead), the client must already have a primary contact, otherwise you'll see "Client does not have a primary contact." or "Client not found."
...
- **Resolution:** Use a sales role (Admin, Sales Admin, or Sales Member). For an automatic create, make sure the client has a primary contact first.
