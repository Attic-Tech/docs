...
description: "How a recorded payment is split across one or more invoices, how over- and under-payment behave, how refunds raise a balance back, and how credits use the same balance math."
keywords: ["payment allocation","record payment","split payment","invoice balance","partial payment","overpayment","refund","credit note","apply credit"]
---
...
- **Recording a new payment:** eligible invoice statuses are **Draft**, **Open**, and **Partially paid**. Recording a payment on a draft promotes it out of Draft (see below).
...
A **voided** invoice can never receive a payment. A **draft** can — recording a new payment on it promotes it out of Draft and applies the payment.
</Card>
...
| Invoice was voided | Unchanged — a payment never flips a voided invoice |
| Invoice was a draft | Promoted out of Draft, then follows the balance rows above — **Partially paid** or **Paid** |
...
Refunds apply to payments taken online. You issue them from your Stripe Dashboard rather than in the product, and they flow back automatically: the refund is recorded against the original payment, allocated to the invoices that payment covered, and every affected invoice is recalculated. The refund appears in the invoice's Transaction History as its own row with a negative amount.
...
- **Allocations must sum to the payment total** (rejected if not) and **target eligible invoices** (Draft, Open, or Partially paid for a new payment — a draft is promoted out of Draft; also Paid when editing).
...
- **Refunds add the amount back.** A refund allocation raises the invoice balance, so a **Paid** invoice can move back to **Partially paid** or **Open**.
