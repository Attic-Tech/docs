...
- **Credits cannot exceed the remaining balance.** A credit allocation larger than the invoice's remaining balance is rejected, with a message naming the invoice and its remaining balance — the same guard payments now apply.
...
| A payment or credit allocation exceeds the balance | The allocation amount exceeds the remaining balance on the invoice. |
...
- **Under-payment is normal.** **Over-payment is blocked** on both the payment and credit paths — an allocation can't exceed what the invoice still owes.
